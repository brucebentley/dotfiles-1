...
###
appname="backlight"
provider="${BLOCK_INSTANCE:-intel_backlight}"
format="{percent}%"
...
arg_desc_long=(
"Choose the backlight provider under /sys/class/backlight/. This can also be set via i3blocks 'instance=' value. If neither is set, it will default to 'intel_backlight'."
)
...
custom_action() {
  ...
  if [ ! -d "${path}/${provider}" ]; then
    echo "Error, provider '${provider}' does not exist in: ${path}"
    exit 1
  fi
  max="$( cat "${path}/${provider}/max_brightness" )"
  cur="$( cat "${path}/${provider}/actual_brightness" )"
  percent="$(( 100 * cur / max ))"
}
