...
function! s:open_on_github(file) abort
  let l:git_dir=emanon#git#get_git_dir(a:file)
  if l:git_dir == ''
    call emanon#functions#echoerr('No .git directory found above ' . a:file)
    return
  endif
...
function! emanon#commands#open_on_github(...) abort
  if a:0 == 0
    let l:files=[expand('%')]
  else
    let l:files=a:000
