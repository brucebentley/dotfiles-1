...
function! emanon#functions#spell() abort
  if has('syntax')
    setlocal spell
...
" Switch to plaintext mode with: call emanon#functions#plaintext()
function! emanon#functions#plaintext() abort
  if has('conceal')
...
function! emanon#functions#keynote() abort
  if has('gui')
    setlocal nonumber
    ...
    echoerr 'emanon#functions#keynote() should be run from within a GUI instance of Vim'
  endif
endfunction

function! emanon#functions#substitute(pattern, replacement, flags) abort
  let l:number=1
...
"
" See also: https://emanon.com/wiki/Sorting_.gitmodules_entries_with_Vim
function! emanon#functions#sortgitmodules(...) abort
  if &ft ==# 'gitconfig' || a:0
    silent %s/\v\n\t/@@@/e
    %sort
    silent %s/\v\@\@\@/\r\t/ge
  else
    echomsg 'Not a "gitconfig" file: use `emanon#functions#sortgitmodules(1)` to force'
  endif
endfunction

" http://stackoverflow.com/a/39348498/2103996
function! emanon#functions#clearregisters() abort
...
function! emanon#functions#echoerr(msg) abort
  try
    echohl ErrorMsg
    echomsg a:msg
  finally
